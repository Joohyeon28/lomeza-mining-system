...
export default function LogDetailModal({
  ...
  useEffect(() => {
    ...
    const tryResolve = async () => {
      if (!entry) return
      if (entry.assets && entry.assets.length && entry.assets[0].asset_code) return // already present
      try {
        const resolveAssetCode = async (id: any) => {
          if (!id) return null
          try {
            const db = getDb()
            const { data } = await db.from('assets').select('asset_code').eq('id', id).limit(1)
            if (data && (data as any[]).length) return (data as any[])[0].asset_code
          } catch (e) {
            // ignore
          }
          ...
          for (const schema of candidateSchemas) {
            try {
              ...
              const { data } = await client.from('assets').select('id, asset_code').eq('id', id).limit(1)
              if (data && (data as any[]).length) return (data as any[])[0].asset_code
            } catch (e) {
              // ignore and continue
            }
        ...
        const code = await resolveAssetCode(entry.machine_id)
        if (!active) return
        if (code) setResolvedAssetCode(code)
      } catch (e) {
        // ignore
      }
    }
    ...
    const fetchLatestEntry = async () => {
      ...
      try {
        ...
        if (!error && data && (data as any[]).length) {
          ...
          if (reviewerId) {
            try {
              // Try site-local users first, then public.users, then global supabase
              let userRec: any = null
              try {
                const r1 = await getDb().from('users').select('id, full_name, name, email').eq('id', reviewerId).limit(1)
              ...
              if (!userRec) {
                try {
                  ...
                  const r2 = await pub.from('users').select('id, full_name, name, email').eq('id', reviewerId).limit(1)
              ...
              if (!userRec) {
                try {
                  const r3 = await supabase.from('users').select('id, full_name, name, email').eq('id', reviewerId).limit(1)
                  if (!r3.error && r3.data && r3.data.length) userRec = r3.data[0]
                } catch (e) {
                  // ignore
                }
              }
              if (userRec) setReviewerName(userRec.full_name || userRec.name || userRec.email || reviewerId)
            } catch (e) {
              // ignore
            }
          }
